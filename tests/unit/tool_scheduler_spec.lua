...
describe('captured tool scheduler',function()
    ...
    -- ended it holds nothing, so a retry — in its own generation or another —
    -- runs. A tool whose process still runs keeps its claims: that is a real
    -- effect in progress, not a quarantine.
    it('releases a crashed tool\'s claims once its process has ended',function()
        service:execute(gen,spec(),{});done[1](outcome('unknown',true,'unknown'));flush()
        assert.equals(0,service:stats().unknown)
        service:execute(gen,spec('retry'),{});flush();assert.equals(2,starts,'the retry runs')
    end)
    it('holds a running tool\'s claims until its process ends, whatever its outcome',function()
        service:execute(gen,spec(),{});done[1](outcome('unknown',false,'unknown'));flush()
        service:execute(gen,spec('retry'),{});flush();assert.equals(1,starts,'still running: the retry waits')
        done[1](outcome('unknown',true,'unknown'));flush();assert.equals(2,starts,'ended: the retry runs')
    end)
    ...
    it('retires probe timers after five seconds without releasing a running unknown effect',function()
        local op=service:execute(gen,spec(),{});done[1](outcome('unknown',false,'unknown'));flush()
