...
describe("resolve_path_in_cwd", function()
    ...
    it("resolves a missing nested path without creating directories", function()
        local abs, err = dispatcher.resolve_path_in_cwd("nonexistent_dir/new.txt", cwd)
        assert.equals(canonical(cwd) .. "/nonexistent_dir/new.txt", abs)
        assert.is_nil(err)
        assert.is_nil(vim.loop.fs_stat(cwd .. "/nonexistent_dir"))
    end)
