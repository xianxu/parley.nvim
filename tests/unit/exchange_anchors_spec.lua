...
describe("exchange_anchors", function()
    ...
    it("counts real anchor resolutions and skips cached resolutions", function()
        ...
        anchors.span(buf, 1, 3)
        assert.equals(3, counter:snapshot().anchors_resolved)
        anchors.span(buf, 2, 3)
        assert.equals(3, counter:snapshot().anchors_resolved)
        vim.api.nvim_buf_set_lines(buf, 2, 3, false, { "changed" })
        anchors.span(buf, 2, 3)
        assert.equals(6, counter:snapshot().anchors_resolved)
