...
describe('document user transactions',function()
    ...
    it('keeps captured source valid through lexical materialization and semantic publication',function()
        local doc,fake=attach({'💬: question','selected','🤖: answer','body'})
        local token=capture(doc,{region(1,0,1,8)})
        assert.equals('idle',D.drain(doc,1000).status)
        assert.is_table(D.resolve_user(doc,token))
        assert.equals('applied',D.apply_user(doc,token,{patches={{region=1,text='updated'}}}).status)
        assert.equals('updated',fake.lines[2]);D.detach(doc)
    end)
    it('admits at most 64 live guards atomically and shares extension guards',function()
        local doc,fake=attach({'source','footer'})
        local tokens={}
        for i=1,63 do tokens[i]=capture(doc,{region(0,0,0,6)}) end
        local failed,reason=D.capture_user(doc,{operation='over',regions={region(0,0,0,6),region(1,0,1,6)}})
        assert.is_nil(failed);assert.equals('capacity',reason)
        local extended=assert(D.extend_user(doc,tokens[1],{regions={region(1,0,1,6)}}))
        assert.equals(64,D.user_guard_stats(doc).live)
        local rejected=D.extend_user(doc,extended,{regions={region(1,0,1,6)}})
        assert.is_nil(rejected);assert.is_table(D.resolve_user(doc,extended))
        assert.is_true(D.cancel_user(doc,tokens[1]));assert.equals(64,D.user_guard_stats(doc).live)
        assert.is_true(D.cancel_user(doc,extended));assert.equals(62,D.user_guard_stats(doc).live)
        fake:edit(1,0,1,6,{'FOOTER'})
        assert.is_true(D.user_guard_stats(doc).last_visits<=64)
        D.detach(doc)
    end)
    it('reclaims abandoned tokens and binds empty insertions to their containing row',function()
        ...
        fake:edit(3,0,3,1,{'T'})
        assert.is_nil(D.resolve_user(doc,token));D.detach(doc)
    end)
    it('updates source guards before delivering subscribers and invalidates boundary joins',function()
        local doc,fake=attach({'first','selected','tail'})
        local token=capture(doc,{region(1,0,1,8)})
        local observed
        local off=D.subscribe(doc,function(event)if event.kind=='edit' then observed=D.resolve_user(doc,token)end end)
        fake:set_lines(0,0,{'before'})
        assert.equals(2,observed.regions[1].first.row)
        fake:edit(1,5,2,0,{''})
        assert.is_nil(observed);assert.is_nil(D.resolve_user(doc,token));off();D.detach(doc)
    end)
    it('cannot substitute an identical neighboring row after deleting the captured source',function()
        local doc,fake=attach({'same','same','tail'})
        local token=capture(doc,{region(0,0,0,4)})
        fake:set_lines(0,1,{})
        assert.equals('same',fake.lines[1]);assert.is_nil(D.resolve_user(doc,token));D.detach(doc)
    end)
    it('leaves capacity unchanged on malformed extension and releases guards on lifecycle',function()
        local doc,fake=attach({'source'})
        local token=capture(doc,{region(0,0,0,6)})
        assert.is_nil(D.extend_user(doc,token,{regions={region(0,0,0,2),region(4,0,4,1)}}))
        assert.equals(1,D.user_guard_stats(doc).live)
        fake:reload({'source'})
        assert.equals(0,D.user_guard_stats(doc).live);assert.is_nil(D.resolve_user(doc,token))
        capture(doc,{region(0,0,0,6)})
        D.detach(doc);assert.equals(0,D.user_guard_stats(doc).live)
        assert.is_nil(D.capture_user(doc,{operation='retired',regions={}}))
        assert.is_nil(D.resolve_user(doc,token))
    end)
    it('applies adjacent nonoverlapping regions in one descending transaction',function()
        local doc,fake=attach({'abcdef'})
        local token=capture(doc,{region(0,0,0,3),region(0,3,0,6)})
        assert.equals('applied',D.apply_user(doc,token,{patches={{region=1,text='A'},{region=2,text='B'}}}).status)
        assert.equals('AB',fake.lines[1]);D.detach(doc)
    end)
    ...
    it('keeps pending lexical reads safe across source capture',function()
