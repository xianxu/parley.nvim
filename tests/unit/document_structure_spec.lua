...
describe('shared live projection queries', function()
    ...
    it('exposes confirmed projections and gates stale semantics for every presentation consumer', function()
        ...
        assert.is_nil(structure.lookup(doc,row.handle,{presentation=true}).metadata.render_before)
        assert.is_nil(structure.lookup(doc,row.handle,{presentation=true}).metadata.presentation)
