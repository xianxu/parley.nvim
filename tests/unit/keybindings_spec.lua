...
describe("key bindings help", function()
    it("includes default key binding entries", function()
        ...
        assert.is_true(has_line(lines, "<C-r>", "Move selected chat"))
        assert.is_true(has_line(lines, "<C-a>", "Cycle note recency window left"))
        assert.is_true(has_line(lines, "<C-s>", "Cycle note recency window right"))
