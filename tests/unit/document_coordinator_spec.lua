...
describe('indexed display byte positions',function()
    it('uses byte columns across UTF-8 rows and refuses opaque or invalid offsets',function()
        local d=attach({'λ界','next'})
        assert.is_nil(D.byte_position(d,0));settle(d)
        assert.same({row=0,col=0},D.byte_position(d,0))
        assert.same({row=0,col=2},D.byte_position(d,2))
        assert.same({row=0,col=5},D.byte_position(d,5))
        assert.same({row=1,col=0},D.byte_position(d,6))
        assert.is_nil(D.byte_position(d,11));assert.is_nil(D.byte_position(d,-1))
        assert.is_nil(D.byte_position(d,math.huge));D.detach(d)
        assert.is_nil(D.byte_position(d,0))
    end)
end)
