...
describe("document sequence", function()
    ...
    it("locates byte boundaries and opaque interiors with bounded navigation", function()
        ...
        local a,offset=S.at_byte(seq,3);assert.equals(0,a.start_row);assert.equals(3,offset)
        a,offset=S.at_byte(seq,5);assert.is_true(a.opaque);assert.equals(0,offset)
        a,offset=S.at_byte(seq,104);assert.equals(99,offset)
        assert.is_nil(S.at_byte(seq,105))
        local initial=S.stats(seq)
        local absent,reason=S.at_byte(seq,3,{nodes=0,entries=0})
        assert.is_nil(absent);assert.equals("budget",reason)
        assert.same(initial,S.stats(seq))
        assert.has_error(function()S.at_byte(seq,0,{nodes=math.huge})end)
        assert.has_error(function()S.at_byte(seq,-1)end)
        local big=S.new(rows(50000));S.stats(big,true)
        a=S.at_byte(big,S.size(big).bytes-1,{nodes=64,entries=16})
        assert.equals(49999,a.start_row)
        local work=S.stats(big);assert.is_true(work.nodes_visited<=64);assert.is_true(work.entries_visited<=16)
    end)
