...
describe("attempt reducer", function()
    ...
    describe("stop escalation", function()
        it("live + stop: KILL is due at exactly stop+2000, then visible at stop+5000", function()
            local state = attempt.transition(spawned(), { type = "stop_requested", now = 1000 })
        ...
        it("live + deadline: the same window, with cause deadline", function()
            local state = attempt.transition(spawned(), { type = "deadline", now = 0 })
        ...
        it("stopping + stop or deadline again: kill_due and the cause are kept", function()
            ...
            state = attempt.transition(state, { type = "stop_requested", now = 1500 })
            assert.equals(2000, state.kill_due)
            state = attempt.transition(state, { type = "deadline", now = 1800 })
            assert.equals(2000, state.kill_due)
            assert.equals("stop", state.stop_cause)
            local _, seen = drive(state, 10000)
            assert.same({ 2000 }, seen.escalate)
        ...
        it("names the cause only of a kill it signalled before the exit", function()
            local state = attempt.transition(spawned(), { type = "stop_requested", now = 0 })
            ...
            local exited = attempt.transition(spawned(), { type = "exit", code = 0 })
            exited = attempt.transition(exited, { type = "stop_requested", now = 0 })
            exited = attempt.transition(exited, { type = "signal_observation", observation = "accepted", signal = 15 })
            assert.is_nil(attempt.kill_cause(exited), "the process had already exited on its own")
            local refused = attempt.transition(spawned(), { type = "stop_requested", now = 0 })
            refused = attempt.transition(refused, { type = "signal_observation", observation = "missing", signal = 15 })
            assert.is_nil(attempt.kill_cause(refused))
        end)
    end)
