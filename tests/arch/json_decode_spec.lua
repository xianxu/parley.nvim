-- #261 M1 review BR-16: every JSON decode in lua/ — `vim.json.decode` or
-- `vim.fn.json_decode`, the two APIs that do the job — decodes input from
...
local function decodes(line)
    if line:match("^%s*%-%-") then return false end
    return line:find("vim.json.decode", 1, true) or line:find("vim.fn.json_decode", 1, true)
end
    ...
    it("finds the decodes it checks", function()
        ...
        for _, file in ipairs(arch.worktree_files({ "lua/**/*.lua" })) do
            for _, line in ipairs(vim.fn.readfile(file)) do
                if decodes(line) then count = count + 1 end
            end
        end
