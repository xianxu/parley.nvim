...
-- #206 moves detailed shortcuts from README into the callable atlas. Keep
-- BR-61's protection against silently dropping the headline actions there.
describe("published shortcut reference agrees with bindings (#206, #214 BR-61)", function()
    it("documents the headline chords and resolves each to a real binding", function()
        setup()
        local bound = {}
        for _, e in ipairs(reg.entries) do
            for _, k in ipairs(reg.resolve_keys(e, parley.config) or {}) do
        ...
        local reference = assert(require("parley.help").read("atlas/ui/keybindings"))
        for _, key in ipairs({ "<M-q>", "<M-CR>", "<M-i>", "<M-p>", "<C-g>?", "gf" }) do
            assert.is_truthy(reference:find("`" .. key .. "`", 1, true),
                key .. " is missing from the published shortcut reference")
            assert.is_true(bound[canon(key)] == true, key .. " resolves to no binding")
        end
    end)
end)
