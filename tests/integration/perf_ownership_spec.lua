...
describe("ownership performance baselines", function()
    it("preserves existing folds without walking exchange anchors on a body edit", function()
        local small, large = probe(100), probe(1000)
        assert.equals(0,small.fold_work.fold_groups_visited)
        assert.equals(0,small.fold_work.native_fold_ops)
        assert.equals(0,large.fold_work.anchors_resolved)
        assert.is_true(small.fold_work.index_nodes_visited>0)
        assert.is_true(large.fold_work.index_nodes_visited<4096)
    ...
    it("includes measured ownership phases at every report size", function()
        ...
        for _, n in ipairs({ 100, 1000, 5000 }) do
            for _, phase in ipairs({ "fold_maintenance", "stream_human_interleave" }) do
                ...
                assert.is_true(found.work.index_nodes_visited > 0)
            end
        end
    end)
    ...
    it("interleaves the production stream handler with real typed-ahead input", function()
        ...
        assert.is_true(sample.stream_work.index_nodes_visited > 0)
    end)
end)
