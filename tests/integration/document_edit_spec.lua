describe('document editor', function()
    ...
    it('invalidates joining after a real file reload',function()
        vim.api.nvim_set_current_buf(buf)
    ...
    it('groups edits in the target buffer without changing the current buffer history',function()
        ...
        assert.equals('α',first_line())
        assert.equals('other',vim.api.nvim_buf_get_lines(other,0,1,false)[1])
        vim.api.nvim_buf_delete(other,{force=true})
    end)
    it('never joins beyond native mutate-then-error or nested unexpected edits',function()
        vim.api.nvim_set_current_buf(buf)
        local plan=append_owned(editor,buf,'a','g','A')
        local original=editor.driver.set_text
        for _,mode in ipairs({'error','nested'}) do
            editor.driver.set_text=function(...)
                original(...)
                if mode=='error' then error('injected post-write failure') end
                local line=first_line()
                original(buf,0,#line,0,#line,{'human'})
            end
            local line=first_line()
            local result=editor:apply({epoch=7,generation='a',grant='g',patches={
                {start={row=0,col=#line,byte=#line},finish={row=0,col=#line,byte=#line},expected_old='',text='B'},
            }},function() return true end)
            editor.driver.set_text=original
            assert.equals(mode=='error' and 'error' or 'interrupted',result.status)
            assert.is_false(editor:can_join_undo(plan))
            assert.equals(mode=='error' and 1 or 2,#result.receipts)
        end
    end)
    it('detaches logically once and retires its native listener on the next event',function()
        assert.is_true(editor:detach())
        assert.equals('detach',events[#events].kind)
        local count=#events
        assert.is_false(editor:detach())
        assert.equals(count,#events)
        local fresh_events={}
        local fresh=E.new(buf,{epoch=8,on_event=function(event) fresh_events[#fresh_events+1]=event end})
        assert.is_true(fresh:attach())
        vim.api.nvim_buf_set_text(buf,0,0,0,0,{'new'})
        assert.equals(count,#events)
        assert.equals(1,#fresh_events)
        assert.equals(8,fresh_events[1].epoch)
        assert.equals('edit',fresh_events[1].kind)
        assert.is_false(editor:can_join_undo({epoch=7,generation='a',grant='g'}))
        fresh:detach()
        assert.equals(2,#fresh_events)
        assert.equals('detach',fresh_events[2].kind)
    end)
end)
