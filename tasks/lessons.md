...
- **Paths in chat files must be relative to the containing file, not Neovim's cwd.** Parley chats are accessible from any working directory — that's a core design goal. So paths written into files (branch links `🌿:`, inline links `[🌿:text](path)`, `@@` references) must resolve from the containing file's directory. Use `:t` (bare filename) when files share a directory. Never use `:~:.` which is cwd-relative — it works during the session that created the link but silently breaks when opened from a different cwd.
