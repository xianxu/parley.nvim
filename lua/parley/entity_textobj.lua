...
--- Parsed chat for `buf`, or nil when this is not a chat buffer (a plain
--- markdown buffer still gets the section and paragraph kinds).
...
function M.select(scope, inner)
	...
	local parley = require("parley")
	local parsed = M.parsed_for(buf, lines)
