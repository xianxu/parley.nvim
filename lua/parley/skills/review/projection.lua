...
local function bufstate(buf)
    _state[buf] = _state[buf] or { records = {} }
    return _state[buf]
end
...
function M.record(buf)
    bufstate(buf).records[hash(buf)] = skill_render.snapshot(buf)
end
...
function M.record_empty_for(buf, base_content)
    bufstate(buf).records[vim.fn.sha256(base_content or "")] = { hl_lines = {}, diags = {} }
end
...
function M.project(buf)
    ...
    if M.decide(s.records, h) == "restore" then
        ...
        s.records[h] = skill_render.snapshot(buf) -- novel forward state (B) → snapshot riding decos
    end
end
...
--- Forget a buffer's projection state (tests / buffer teardown).
