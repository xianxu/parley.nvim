...
- **fold_projection** — gains `anchor_kind` (block kind → the structural kind its first line must classify as), `verify_anchors(ranges, lines, patterns)` → `ok, failed_index` (anchor **and** interior), `verify_starts(starts_0, lines, patterns)` validating a model's exchange starts before they become extmark identity, `ranges_within(ranges, first_0, last_0)` tying creation to destruction (a range must lie inside the rows identity says the exchange owns), and `is_foldable(kind)` as a read-only view of the policy. `FOLDABLE` is derived from `ANCHOR_KIND` rather than restated, so the two key sets cannot drift — drift there is silent and total (a nil `anchor_kind` fails verification and refuses the whole exchange). Stays pure and nvim-free. *(An interim `verify_span` guarded the destructive half in rounds 1–4; it was replaced in round 5 when identity took over that job — see Revisions.)*
