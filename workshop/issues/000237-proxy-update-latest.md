...
- 2026-09-12: closed — #237 complete. M1 closed SHIP after four rounds (00dbd43); M2 closed FIX-THEN-SHIP after three rounds with its fixes in the M2 close commit. Update installs the latest release (or cliproxy.download_version) atomically and restarts only the proxy parley started, confirming the version after; status shows the running version against the latest; first-run auto_download follows the same rule. Verified: make test JOBS=4 at 88d97dd, 211 of 212 spec files pass (the one failure, fold_invariants, is an uncommitted chat rename in the operator worktree and passes in a clean worktree); conformance on the real 7.2.158 and 7.1.71 and, under PARLEY_LIVE_GITHUB=1 with nothing on PATH, on the latest release parley downloads itself: the version header keyed and rejected, the management lockout and its ban body, both chat routes, the header with management off and the live redirect all pass; the two #205 catalog cases fail on every build with the fabricated credential (pre-existing, follow-up); lint 0/0. Operator live check: update installed 7.2.159, then claude, the smoke test and a Fable chat answer, after the /v1/messages fix.; review verdict: SHIP
