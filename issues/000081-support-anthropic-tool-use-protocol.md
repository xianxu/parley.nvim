...
- **M2 Task 2.7 DEFERRED to #90 renderer refactor** (2026-04-09): Three rounds of manual testing surfaced a compounding bug pattern in `chat_respond.M.respond`'s buffer-position arithmetic (`response_line / progress_line / response_start_line / raw_request_offset / in_tool_loop_recursion` branching with `+3` vs `+1` magic offsets). Fixed two rounds (progress_line offset, stuck-spinner skip-on-recursion) before a third issue emerged: Anthropic rejects the recursive call with `invalid_request_error: does not support assistant message prefill`, while the saved payload at `~/.cache/nvim/parley/query/...8f0d52.json` looks spec-correct (3 messages ending in user[tool_result]). Root cause is almost certainly in the renderer/mutation path rather than `_build_messages` (verified via offline debug harness). Rather than stack a fourth offset patch, filed #90 to extract a pure `exchange → lines` + `positions` layer and single-entry-point buffer mutation. M2 Task 2.11 code review gate also postponed until #90 lands. M1/M2 code that survives #90 unchanged: tools/*, providers encode/decode, tool_loop module, chat_parser content_blocks, `_build_messages` + `_emit_content_blocks_as_messages`, and all associated tests. Only the buggy ~150 lines of `M.respond` renderer arithmetic get replaced.
