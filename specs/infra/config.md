...
- `chat_finder_mappings.move`: Picker-local shortcut (default `<C-g>m`) for moving the selected chat to another registered chat root without colliding with picker confirm on `<CR>`.
