...
- Default agents: GPT5.4 (openai), Claude-Sonnet (anthropic), ClaudeAgentTools (anthropic, all 6 builtin tools), Gemini2.5-Pro (googleai), Proxy-GPT5.4 (cliproxyapi), Claude-Code (cliproxyapi/code_execution_20260120)
...
- Virtual text on first chat line: `[AgentName]`. Indicator badges render as a single `[...]` group appended after the name: `🔧` when `tools` is non-empty, `🌎` when web_search is enabled and supported (`🌎?` when unsupported). Combined example: `ClaudeAgentTools[🔧🌎]`. Helpers `highlighter.agent_tool_badge` / `agent_web_search_badge` are the single source, shared by picker, lualine, and the buffer-top extmark.
