...
return {
    {
        ...
        config = function()
                ...
                mapping = cmp.mapping.preset.insert {
                    ...
                    ['/'] = cmp.mapping(function(fallback)
                      ...
                      if cmp.visible() then
                        ...
                        local in_markdown = vim.bo.filetype == 'markdown'
                        ...
                        if in_markdown and has_path then
                          -- Allow `/` to be inserted normally
                          fallback()
                        else
                          cmp.close()
                          fallback()  -- also insert `/` when closing
                        end
                      else
                        fallback()
                      end
            ...
            -- for markdown files use path completion
            cmp.setup.filetype('markdown', {
              completion = {
                -- Allow `/`, `.`, and `~` to be part of completion trigger
                keyword_pattern = [[\~\?\(\k\|[\/\.\-]\)\+]],
                keyword_length = 1,
              },
              sources = cmp.config.sources({
